...
Self-contained vault for sensitive values — Rust, with no dependency on any
hosted manager or external service. Encryption at rest, multi-recipient
sharing, scoped service-account grants, recovery, tamper-evident audit, and
runtime injection. All cryptography is delegated to vetted local tools (gpg,
openssl, shasum, and optional oathtool); none is hand-rolled.
...
The binary is `target/release/skarbiec`. It shells out to `gpg`, `openssl`,
and `shasum` at runtime, so those must be on `PATH`.

## Quickstart

```sh
skarbiec init <owner-uid>     # create the vault, sealed to your gpg key
skarbiec generate --length N  # make a login string from OS entropy
skarbiec list                 # item metadata (never values)
skarbiec serve --port <p>     # loopback HTTP API for local clients
skarbiec mcp                  # stdio MCP server for agents
```

The on-disk file at `SKARBIEC_VAULT_FILE` (default `skarbiec.vault.json`) is
armored ciphertext; it is safe at rest. `.gitignore` keeps every vault, audit
