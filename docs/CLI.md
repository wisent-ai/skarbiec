...
| `token-ensure-read <consumer> <item> --field <field> --token-file <path>` | Idempotently add one exact field read to an existing direct grant without rotating its bearer, replacing its TTL, replaying its other capabilities, or dropping stale rows. The owner-only token file must still hash to that consumer's recorded bearer. |
