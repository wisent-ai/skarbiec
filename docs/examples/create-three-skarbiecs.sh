...
SKARBIEC_VAULT_FILE=~/.skarbiec-osobisty.vault.json "$SB" set konto-bank --type login login_email=moj@email.pl login_password="$EXAMPLE_SECRET"

# isolation proof: the item exists in one vault, not in the others
SKARBIEC_VAULT_FILE=~/.skarbiec-osobisty.vault.json "$SB" get konto-bank --field login_email
