...
# Source identity survives Stado's immutable archive through export-subst.
commit=""
commit_file="$source_dir/.release-commit"
if [[ -r "$commit_file" ]]; then
  substituted=$(tr -d '[:space:]' <"$commit_file")
  if [[ "$substituted" =~ ^[0-9a-f]{40}$ ]]; then commit="$substituted"; fi
fi
if [[ -z "$commit" && -d "$source_dir/.git" ]]; then
  commit=$(git -C "$source_dir" rev-parse HEAD)
fi
if [[ ! "$commit" =~ ^[0-9a-f]{40}$ ]]; then
  printf 'cannot name the source revision for %s: %s has no commit\n' "$version" "$commit_file" >&2
  exit 67
fi
...
SKARBIEC_RELEASE_COMMIT="$commit" CARGO_TARGET_DIR="$build_root" \
