...
fn main() -> Result<()> {
    ...
    match command.as_str() {
        ...
        "help" => emit(
            &json!({"groups": ["grant","route","credential"], "commands": ["status","doctor","recover-daemons","vaults","init","set","set-json","get","list","duplicates","stamp-fingerprints","retag","rename","backfill-item-uids","delete","reclaim","restore","purge","restore-version","generate","import","migrate","migrate-v2","add-user","rotate-owner","share","revoke","users","export-key","grant","acquisition-request","acquisition-read","key-doctor","recovery-status","recovery-drill","emergency-grant","emergency-cancel","emergency-list","emergency-activate","policy-set","policy-get","policy-check-length","audit","audit-query","audit-epoch-start","verify-chain","route","totp","totp-seed-state","breach-check","sync-init","sync-push","sync-pull","pull","donate","donations","donation-accept","donation-reject","enroll","sync-daemon","sync-status","bond-add","bond-list","bond-remove","capability-serve","capability-status","credential","apple-challenge-put","version"]}),
        ),
        "mcp" => net::mcp::serve(),
