// Loopback HTTP API for metadata, legacy resolve, and one-time field
// acquisitions. A request-only bootstrap can issue but never directly read;
// the field-bound bearer is atomically consumed on its first successful read.
...
fn handle(mut stream: TcpStream) -> Result<()> {
    ...
    if method == "POST" && path == "/v1/acquisitions" {
        let Some((item, field)) = request_target(&body) else {
            return write_response(
                &mut stream,
                bad_line,
