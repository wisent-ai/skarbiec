...
mod tests {
    ...
    #[test]
    fn canonical_journey_is_closed_and_reaches_first_success() {
        let definition = canonical_definition().expect("canonical journey");
        ...
        loop {
            assert!(visited.insert(current.clone()), "journey contains a cycle");
            let screen = screen_by_id(&definition, &current).expect("reachable screen");
            let Some(next) = next_screen_id(screen).expect("valid transition") else {
                assert_eq!(
                    screen.get("screen_kind").and_then(Value::as_str),
                    Some("first_success")
                );
                break;
            };
    ...
    #[test]
    fn canonical_evidence_is_required_exactly() {
        let definition = canonical_definition().expect("canonical journey");
        let safe_demo = screen_by_id(&definition, "safe_demo").expect("safe demo");
        assert!(!evidence_satisfied(safe_demo, &Map::new()).expect("evidence check"));

        let wrong = Map::from_iter([("demo_item_read".to_string(), Value::Bool(false))]);
        assert!(!evidence_satisfied(safe_demo, &wrong).expect("evidence check"));

        let exact = Map::from_iter([("demo_item_read".to_string(), Value::Bool(true))]);
        assert!(evidence_satisfied(safe_demo, &exact).expect("evidence check"));
    }

    #[test]
    fn demo_item_id_is_stable_and_bounded() {
        assert_eq!(
            demo_item_id("abcdef0123456789"),
            "onboarding-safe-note-abcdef01"
        );
    }
}
