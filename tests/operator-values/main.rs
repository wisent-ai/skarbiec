...
fn operator_get_returns_full_item_when_no_field_specified() {
    let mut fixture = CliFixture::new("operator-values");
    ...
    assert!(stdout.contains("JBSWY3DPEHPK3PXP"), "totp_secret should be in output");
}
...
fn operator_totp_generates_code_when_seed_exists() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("has_seed") && stdout.contains("true"), "should indicate seed exists");
...
fn operator_totp_indicates_no_seed_when_field_missing() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("has_seed") && stdout.contains("false"), "should indicate no seed");
}
...
fn update_credential_fields_preserves_all_original_fields() {
    ...
    let updated_json = item_json
        .replace("pass222", "newpass222")
        .to_string();
    ...
    assert!(final_json.contains("frank"), "username should survive update");
    assert!(final_json.contains("newpass222"), "password should be updated");
    assert!(final_json.contains("JBSWY3DPEHPK3PXR"), "totp_secret should survive update");
}
